...
const indexSO = async (req, res) => {
  try {
      ...
      // Count total records
      const totalRecords = await knex('l_sales_orders').join('l_quotations','l_quotations.quotation_id','l_sales_orders.quotation_id')
      ...
      let filteredRecordsQuery =  knex('l_sales_orders').join('l_quotations','l_quotations.quotation_id','l_sales_orders.quotation_id').join('l_customers','l_customers.customer_id','l_quotations.customer_id')
      .whereRaw('l_quotations.client_id= '+id+'');
...
const indexSOId = async (req, res) => {
  try {
      const id = req.data.client_id;
      const token = req.query.token;
      // Count total records
      var data = await knex.table('l_sales_orders').join('l_quotations','l_quotations.quotation_id','l_sales_orders.quotation_id')
        .join('l_customers', 'l_customers.customer_id', 'l_quotations.customer_id')
        .where({
            so_token: token,
        });
...
const createSalesOrder = async(req, res ) => {
  try {
      ...
      var stats = await knex('l_quotations').update({
        quotation_status: 5
      }).where({
        quotation_id:req.body.quotation_id
      })
      const [dayDate, monthDate, yearDate] = req.body.so_date.split('-');
      const f1 = `${dayDate}-${monthDate}-${yearDate}`;
      var l_quotation = await 
          knex('l_sales_orders').insert({
            so_date: f1,
            so_status: 1,
            so_token: randomString,
            so_po_customer: req.body.so_po_customer,
            created_by: user_id,
            quotation_id: req.body.quotation_id
          })
      res.status(200).send(
...
const addSubSalesOrder = async (req, res) => {
  try {
      const id = req.data.client_id;
...
const updateSalesOrder = async(req, res ) => {
  try {
      ...
      var l_quotation = await 
      knex('l_sales_orders').update({
        so_po_customer     :req.body.status,
      }).where({
        so_token:req.body.id
      })
      res.status(200).send(
        {
          message: "Success",
...
const updateStatusSalesOrder = async(req, res ) => {
  try {
    var status = req.body.status;
    var query = await knex('l_sales_orders').where({
      so_token:req.body.id
    })
    var qs = query[0].so_status
    var q_count = await knex('l_sales_orders').join('l_quotations','l_quotations.quotation_id','l_sales_orders.quotation_id').where({
      client_id:req.data.client_id
    }).andWhereRaw('MONTH(so_date) = MONTH(NOW()) and so_number != ""');
    ...
    var name = ""+cust+"/SO/"+year+"/"+month+"/"+total_out+"";
    if(status==1){ 
      var l_quotation = await 
            knex('l_sales_orders').update({
              so_status:req.body.status
            }).where({
              so_token:req.body.id
            })
    }else if(status==2){
      ...
      if(qs==2){
        if(query[0].so_number=='' || query[0].so_number==null){
          var a = name
        }else{
          var a = query[0].so_name
        } 
        var l_quotation = await knex('l_sales_orders').update({
          so_status:req.body.status,
          so_number:a
        }).where({
          so_token:req.body.id
...
const pdfSalesOrder = async(req, res ) => {
  try {
    var img = req.data.client_main_logo.toString('base64')
  
    var query = await knex('l_sales_orders').join('l_quotations','l_quotations.quotation_id','l_sales_orders.quotation_id').join('l_customers','l_customers.customer_id','l_quotations.customer_id').join('l_tops','l_tops.top_id','l_quotations.top_id')
                .where({so_token:req.query.token})
    var table_data = []
   ...
   const dimensions = imageSize(Buffer.from(im, 'base64'));
    ...
    if(baris>16){
      ...
      doc.setFontSize(9);
      doc.text('berikut item detail dari sales order '+query[0].sales_number+':', 15, 45);
      doc.autoTable({
...
const insertDataSalesOrder = async(req, res ) => {
  try {
      var l_quotation = await 
...
module.exports = { addSubSalesOrder, indexSO ,indexSOId ,createSalesOrder,updateSalesOrder,deleteSalesOrder,updateStatusSalesOrder,pdfSalesOrder,insertDataSalesOrder}